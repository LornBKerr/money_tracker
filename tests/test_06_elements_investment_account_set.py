...
def test_0605_all_rows(create_accounts_table):
    ...
    count_result = dbref.sql_query(
        "SELECT COUNT(*) FROM "
        + account_set.get_table()
        + " WHERE account_type = "
        + str(AccountType.INVESTMENT)
    )
    count = dbref.sql_fetchrow(count_result)["COUNT(*)"]
    assert count == len(account_set.get_property_set())
    close_database(dbref)
...
def test_0606_selected_rows(create_accounts_table):
    dbref = create_accounts_table
    load_accounts_table(dbref)
    account_set = InvestmentAccountSet(
        dbref, "account_subtype", InvestmentAccountType.SINGLE_FUND
    )
    count_result = dbref.sql_query(
        "SELECT COUNT(*) FROM "
        + account_set.get_table()
        + " WHERE account_subtype = "
        + str(InvestmentAccountType.SINGLE_FUND)
    )
    count = dbref.sql_fetchrow(count_result)["COUNT(*)"]
    assert count == len(account_set.get_property_set())
    close_database(dbref)
...
def test_0607_ordered_selected_rows(create_accounts_table):
    ...
    load_accounts_table(dbref)
    account_set = InvestmentAccountSet(
        dbref, "account_subtype", InvestmentAccountType.BROKERAGE, "name"
    )
    count_result = dbref.sql_query(
        "SELECT COUNT(*) FROM "
        + account_set.get_table()
        + " WHERE account_subtype = "
        + str(InvestmentAccountType.BROKERAGE)
    )
    count = dbref.sql_fetchrow(count_result)["COUNT(*)"]
